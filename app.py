...
@app.before_request
def check_auth():
    if request.endpoint and request.endpoint not in ['login', 'static'] and 'password' not in session:
        return redirect(url_for('login', next=request.url))

def validate_key(key, existing_keys):
    """Validate a tag key."""
    ...
    if not re.match(r'^[a-zA-Z][a-zA-Z0-9_:]*$', key):
        return False, "Key must start with a letter and contain only letters, numbers, underscores, and colons"
...
def add_area():
    if request.method == 'POST':
        ...
        try:
            ...
            # Remove any empty keys or keys with only whitespace
            tags = {k.strip(): v for k, v in tags.items() if k and k.strip()}
            if not tags:
                return jsonify({'error': {'message': 'No valid tags provided'}}), 400
        ...
        for key, value in dict(tags).items():
            ...
            # Validate key format
            is_valid_key, key_error = validate_key(key, existing_keys)
            if not is_valid_key:
                validation_errors.append(f"Invalid key '{key}': {key_error}")
                continue
            ...
            if not value or not str(value).strip():
                requirements = type_requirements.get(key, {})
                if requirements.get('required', False):
                    validation_errors.append(f'Empty value for required field: {key}')
                continue

            # Validate value based on type
            requirements = type_requirements.get(key, {'type': 'text'})
...
def search_areas():
    try:
        ...
        result = rpc_call('search_areas', {'query': search_query})
        if 'error' in result:
            return jsonify({'error': result['error']}), 400
...
def rpc_call(method, params):
    ...
    try:
        response = requests.post(f"{API_BASE_URL}/rpc", json=payload, timeout=10)
